...
Target.prototype.onData = function(data) {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (this.lines.length > self.limit) {
      var msg = util.format(
        "ERRORDOG: Too many errors (%d errors in %d seconds), " +
          "further errors will be discarded!", this.lines.length, this.interval);
      this.emitter.emit('alert', 2, [msg]);
      break;
    }
