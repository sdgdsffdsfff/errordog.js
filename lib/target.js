...
Target.prototype.tail = function() {
  ...
  log.info('target %s child process launched', self.name);
};
  ...
  setInterval(function() {
    ...
    while (level >= 0) {
      if ((self.lines.length >= self.thresholds[level]) &&
        ...
        log.debug('target %s emit alert, level: %d, lines: %d',
                 self.name, level, self.lines.length);
        break;
      }
      level -= 1;
    }
...
Target.prototype.connect = function() {
  ...
  self.alerters.forEach(function(item) {
    ...
    log.info('target %s and alerter %s connected',
